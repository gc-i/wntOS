...
from ..view.Ui_DistributionPointDialog import *
...
from ..model.core_classes import CoDistributionPoint
from sqlalchemy.exc import SQLAlchemyError
from ..model.DatabaseHelper import *
from qgis.core import QgsFeatureRequest


class DistributionPointDialog(QDialog, Ui_DistributionPointDialog, DatabaseHelper):

    def __init__(self, layer, feature_id, attr_update=False, parent=None):

        super(DistributionPointDialog, self).__init__(parent)
        ...
        self.damage_form = DamageForm('co_distribution_point', self.__feature_id)
        self.tabWidget.addTab(self.damage_form, 'Damages')
        self.maintenance_form = MaintenanceForm('co_distribution_point', self.__feature_id)
        self.tabWidget.addTab(self.maintenance_form, 'Maintenance')
        self.document_form = DocumentForm('co_distribution_point', self.__feature_id)
        self.tabWidget.addTab(self.document_form, 'Documents')
        self.customer_form = CustomerManagementForm(CoDistributionPoint, self.__feature_id)
        self.tabWidget.addTab(self.customer_form, 'Customers')
    ...
    def __populate_details(self):
        ...
        if feature['dist_point_number']:
            self.dist_point_number_edit.setText(feature['dist_point_number'])
    ...
    def accept(self):
        ...
        self.__layer.beginEditCommand('Update attributes (co_distribution_point)')

        attr_val = self.dist_point_number_edit.text() if len(self.dist_point_number_edit.text()) > 0 else None
        self.__layer.changeAttributeValue(self.__feature_id, feature.fieldNameIndex('dist_point_number'), attr_val)
    ...
    def __validate_user_input(self):
        ...
        self.dist_point_number_edit.setText(self.dist_point_number_edit.text().strip())
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_F1:
            ...
            if tab_index == 0:
                PluginUtils.show_help("Add_Edit_Distribution_Point.htm")
            elif tab_index == 1:
